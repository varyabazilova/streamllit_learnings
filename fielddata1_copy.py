...
st.title("Plot Selected Columns as Numeric")

uploaded_file = st.file_uploader("Upload your CSV file", key = 'new')
...
if uploaded_file is not None:
    ...
    for col in df.columns:
        if st.checkbox(f"Include '{col}'", value=False):
    ...
    uploadedold_file = st.file_uploader("Upload your CSV file", key = 'old')
...
# # Upload old data
# old_file = st.file_uploader("Upload old data (no processing)", type=["csv"], key="old")

# # Upload new data that requires processing
# new_file = st.file_uploader("Upload new data (to be processed)", type=["csv"], key="new")
...
#     # Example: drop NaNs, convert columns, etc.
#     st.write("Before processing (new data):", new_df.head())

#     # Replace this with your real processing logic
#     new_df = new_df.dropna()
#     new_df["processed_col"] = new_df["some_column"] * 2  # Example

#     st.write("After processing (new data):", new_df.head())

#     # Combine the data
#     combined_df = pd.concat([old_df, new_df], ignore_index=True)

#     # Show and/or download
#     st.write("Combined data:", combined_df.head())

#     csv = combined_df.to_csv(index=False).encode('utf-8')
#     st.download_button("Download combined CSV", data=csv, file_name="combined_data.csv", mime='text/csv')
